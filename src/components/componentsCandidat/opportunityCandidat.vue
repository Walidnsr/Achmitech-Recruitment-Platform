<template>
    <div class="cursor-pointer mx-auto mt-12 grid grid-cols-4 gap-2 w-11/12 bg-gray-100 text-black rounded-2xl pl-24 pr-20 py-8 select-none" @click="details=true">
        ...
        <div class="my-auto col-span-2">
            ...
            <p>{{ company.duration }} ago</p>
        </div>
        <div class="my-auto">
            <button @click.stop="details=false" @click="showButton=false" v-if="showButton" class="text-lg rounded-full bg-achmi-primary text-white font-bold px-8 py-2 h-1/2 my-auto ml-20 hover:text-black hover:bg-white border-achmi-primary border-4">To Apply</button>
            <div @click.stop="details=false" v-else class="rounded-full bg-white text-black font-bold text-lg px-8 h-1/2 my-auto ml-20 border-achmi-primary border-4 py-2">Applied</div>
        </div>
    </div>
